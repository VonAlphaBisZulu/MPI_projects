% This script computes MCS for the MaxCat Strain
% It computes the smallest MCS for the weakly and directionally growth-coupled
% and substrate uptake production of isobutanol with E. coli.
...
load('C:\Users\phili\Documents\MATLAB\StrainBooster\johanna\models\ECC2_extended.mat');
load('C:\Users\phili\Documents\MATLAB\StrainBooster\johanna\models\ECC2extended_geneNames.mat');
cnap = CNAcobra2cna(ECC2,0);

% Load MaxCat-Knockouts
load('C:\Users\phili\Documents\MATLAB\StrainBooster\johanna\models\MaxCatKRECC2extFermentationProd14032022.mat');
...
if isfield(full_cnap,'rType')
    intvCost = rkiCost;
    intvCost(isnan(rkiCost)) = rkoCost(isnan(rkiCost));
    intvgCost = gkiCost;
    intvgCost(isnan(gkiCost)) = gkoCost(isnan(gkiCost));
    intvCost = rmap'*intvCost;
    intvCost(full_cnap.rType=='g') = intvgCost;
else
    intvCost = kiCost;
...
%% compute thresholds:
% pGCP: At 20% of maximal growth rate, maximize production rate. 
%       30% of this rate should be possible at maximal growth after 
...
%% 4) Characterization and ranking of MCS and plotting production envelopes
