...
% steps)
%% Test MCS
textprogressbar('Testing MCS: ');
q = parallel.pool.DataQueue;
afterEach(q, @textprogressbar);
parfor i = 1:size(mcs1,2)
    send(q, i);
    cnap_temp = cnap1;
    cnap_temp.reacMin(isnan(mcs1(:,i)) | mcs1(:,i) == -1) = 0;
    cnap_temp.reacMax(isnan(mcs1(:,i)) | mcs1(:,i) == -1) = 0;
    testRegionFeas(cnap_temp,[], cnap1.mcs.T , cnap1.mcs.t , cnap1.mcs.D , cnap1.mcs.d)
    disp(round(i/size(mcs1,2)*100));
end
