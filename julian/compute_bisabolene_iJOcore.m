...
% 2. compute max production at 20% growth (given the DXP pathway is inactive)
cnap.objFunc(:) = 0;
cnap.objFunc(ismember(cnap.reacID,{product_rID})) = -1;
fv_fix = nan(cnap.numr,1);
fv_fix(ismember(cnap.reacID,{biomass_rID})) = r_bm_max20;
[V,v] = genV([{DXP_rID} {'<='} 0.5e-3 ],cnap);
fv = CNAoptimizeFlux(cnap,fv_fix,[],2,-1,0,V,v);
...
for coupling = {'potential growth-coupling' 'weak growth-coupling' 'directional growth-coupling' 'substrate uptake coupling'}
    ...
    [modules{1}.V(2,:),modules{1}.v(2,:)] = genV([{DXP_rID} {'<='} 0.01 ],cnap);
    
    modules{2}.sense = 'desired';
    modules{2}.type  = 'lin_constraints';
    [modules{2}.V(1,:),modules{2}.v(1,:)] = genV([{DXP_rID} {'>='} 0.05 ],cnap);
    ...
    switch coupling{:}
        case 'potential growth-coupling'
            ...
            [modules{1}.V(3,:),modules{1}.v(3,:)] = genV([{product_rID} {'>='} r_p_20 ],cnap);
            [modules{1}.c(1,:),~]                 = genV([{biomass_rID} {'>='} 0    ],cnap); % maximize biomass
        case 'weak growth-coupling'
            modules{3}.sense = 'target';
            modules{3}.type  = 'bilev_w_constr';
            [modules{3}.V(1,:),modules{3}.v(1,:)] = genV([{product_rID} {'<='} r_p_20 ],cnap);
            [modules{3}.V(2,:),modules{3}.v(2,:)] = genV([{biomass_rID} {'>='} 0.05   ],cnap);
            [modules{3}.V(3,:),modules{3}.v(3,:)] = genV([{DXP_rID} {'<='} 0.01 ],cnap);
            [modules{3}.c(1,:),~]                 = genV([{biomass_rID} {'>='} 0      ],cnap); % maximize biomass
            modules_wg = modules;
        case 'directional growth-coupling'
            modules{3}.sense = 'target';
            modules{3}.type = 'lin_constraints';
            [modules{3}.V(1,:),modules{3}.v(1,:)] = genV([{[product_rID ' / ' biomass_rID]}   {'<='}  Y_PBM ],cnap);
            [modules{3}.V(2,:),modules{3}.v(2,:)] = genV([{biomass_rID}   {'>='}  0.01 ],cnap);
            [modules{3}.V(3,:),modules{3}.v(3,:)] = genV([{DXP_rID} {'<='} 0.01 ],cnap);
...
filename = [fileparts(which('compute_bisabolene.m')) filesep 'results' filesep product_rID '_iJO'];
