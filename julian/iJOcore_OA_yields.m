...
% conserve growth-essential
cnap1 = cnap;
cnap1.reacMin(cnap1.reacMin>=0) = 0; % deactivate ATPM
r_BM = find(~cellfun(@isempty,regexp(cellstr(cnap.reacID),'BIOMASS_Ec.*core.*','match')));
cnap1.objFunc(:) = 0;
cnap1.objFunc(r_BM) = -1;
fv = CNAoptimizeFlux(cnap1,[],[],2,0);
A_ieq = sparse(1,r_BM,-1,1,cnap1.numr);
b_ieq = -fv(i);
[minFlux,maxFlux] = CNAfluxVariability(cnap1,[],[],-1,1:cnap1.numr,A_ieq,b_ieq,0);
growth_essential = find((minFlux.*maxFlux)>0);
core_reacs = unique([core_reacs;cellstr(cnap1.reacID(growth_essential,:))]);
