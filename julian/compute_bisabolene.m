    ...
    for i = 1:num_iter
        if solve_in_new_process    
            ...
            if gene_mcs  %#ok<*UNRCH>
               [rmcs, gmcs, comptime(i)] = MCS_enum_thread(gene_mcs,cnap,modules,...
                                                rkoCost,...
                                                maxSolutions,maxCost,...
                                                gkoCost,...
                                                options,verbose);
            else
                [gmcs, comptime(i)] = MCS_enum_thread(gene_mcs,cnap,  modules,...
                                                koCost,...
                                                maxSolutions,maxCost,...
                                                [],...
                                                options,verbose);
...
function [rmcs, gmcs, comptime] = MCS_enum_thread(gene_mcs,cnap,modules,...
                                            rkoCost,...
                                            maxSolutions,maxCost,...
                                            gkoCost,...
                                            options,verbose)
        ...
        cd(wd);
        load(filename,'gmcs','rmcs','comptime','status');
        if status ~= 0
            comptime = nan;
        end
        rmdir(wdir,'s');
