function [cnap, mcs, indexUptakeReacs, T, t, D, d,notknockable,reacNames,reg,sys] = CNAenumeratecMCSxlsExp( cnap, xls_product_filename, xls_substrate_filename , maxMCSnum, maxMCSsize,output_filename)
%% Add new reactions to model
try
    cnap = CNAaddSpecsAndReacsFromFile(cnap,xls_product_filename);
catch
    warning([xls_product_filename ': nothing was added to model']);
end
                   
%% read out cMCS-Calculation parameters
TableReads = loadSpecReacXLStoStrArray(xls_product_filename);
...
%% Add supplementary substrates to choose from
indexUptakeReacs = [];
if ~isempty(xls_substrate_filename)
    for i = 1:size(xls_substrate_filename,1)
        try
            cnap = CNAaddSpecsAndReacsFromFile(cnap,strtrim(xls_substrate_filename(i,:)));
            % Find reaction index of substrate uptake reaction
            TableReads = loadSpecReacXLStoStrArray(strtrim(xls_substrate_filename(i,:)));
            [ridrow,ridcol,reacSheet]  = ind2sub(size(TableReads),find(strcmp(strtrim(TableReads),'reac_id')));
            try
                indexUptakeReacs = [indexUptakeReacs find(strcmp(cellstr(cnap.reacID),TableReads(ridrow+1,ridcol,reacSheet)))];
            catch
                % if it cannot be found, don't add anything (can also
                % happen when 'reac_id' cannot be found in xls sheet)
            end
        catch
            continue;
        end
...
% Tke 
% cnap.reacMin(regulation.reg_ind) = -10;
% cnap.reacMax(regulation.reg_ind) = 0;
...
if ischar(output_filename) &&  ~isempty(output_filename)
    [mcs,reacNames,reg,sys] = CNAregMCSEnumerator2(cnap,T,t,D,d,notknockable,maxMCSnum,maxMCSsize,output_filename,0,[],[],[],[],indexUptakeReacs);%regulation
else
    [mcs,reacNames,reg,sys] = CNAregMCSEnumerator2(cnap,T,t,D,d,notknockable,maxMCSnum,maxMCSsize,[],0,[],[],[],[],indexUptakeReacs);
end
% mcsReadable zeigt die MCS an mit 1 an den knock-outs und mit -1 an den
% knock-ins
mcsReadable = zeros(size(mcs));
mcsReadable(:,indexUptakeReacs) = 1;
mcs = mcs - mcsReadable;
%cnap.reacMax(indexUptakeReacs) = 0;
cnap.reacMin(indexUptakeReacs) = 0;
